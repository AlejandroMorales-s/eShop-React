...
export default function Form({ setShowingModal, setError }) {
  ...
  return (
    <form
      onSubmit={accountCreated}
      className="flex flex-col w-100 max-w-45 m-auto"
    >
      <div>
        <label
          htmlFor="name"
      ...
      <div>
        <label
          htmlFor="password"
          className="text-bold font-medium dark:text-gray text-boldText flex flex-col gap-y-0.5 mb-[5px]"
        >
          Password
          <div className="relative w-100">
            <input
              ...
              type="password"
              placeholder="********"
              className={`w-100 border-primary border-2 rounded px-0.5 h-4.5 shadow-shadow dark:bg-darkBg dark:border-primary-light dark:text-gray focus:ring-1 focus:outline-none ${
                validPassword
                  ? "focus:border-green focus:ring-green dark:focus:border-green dark:focus:ring-green"
                  : "focus:border-yellow focus:ring-yellow dark:focus:border-yellow dark:focus:ring-yellow"
              }`}
            />
            {isPasswordVisible ? (
              <svg
                ...
                className=" font-semibold text-darkBg dark:text-gray h-6 w-6 absolute -translate-y-2/4 top-2/4 right-0.5"
                viewBox="0 0 20 20"
                fill="currentColor"
              >
        ...
        <div
          className={`${
            validPassword ? "bg-green" : "bg-yellow"
          } transition-all delay-100 ease-out h-fit -translate-y-0.5 px-1 py-0.5 ${
            inputClicked ? "relative opacity-100" : "absolute opacity-0"
        ...
        >
          {validPassword ? (
            ...
            <p>
              8-16 characters, at least one digit, one lower and upper case
              letter 😢
            </p>
          )}
        </div>
      ...
      <div>
        ...
        >
          ...
          <input
            ...
            className={` border-primary border-2 rounded px-0.5 h-4.5 shadow-shadow dark:bg-darkBg dark:border-primary-light dark:text-gray focus:ring-1 focus:outline-none ${
              passwordsMatch
                ? "focus:border-green focus:ring-green dark:focus:border-green dark:focus:ring-green"
                : "focus:border-red focus:ring-red dark:focus:border-red dark:focus:ring-red"
            }`}
          />
          <div
            className={`${
              passwordsMatch ? "bg-green" : "bg-red"
            } transition-all delay-100 ease-out h-fit -translate-y-0.5 px-1 py-0.5 ${
              inputClicked ? "relative opacity-100" : "absolute opacity-0"
            } rounded rounded-bl-xl`}
          >
