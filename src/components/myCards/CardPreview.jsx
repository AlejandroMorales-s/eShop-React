...
export default function CardPreview({
  name, number, month, year, type, ccv, rotate,
}) {
  if (number.slice(0, 1) === "4") type = "Visa";
  else if (number.slice(0, 1) === "5") type = "Mastercard";
  else type = "";
  ...
  return (
    <div id="rotate" className={`${rotate && "active"} card m-auto my-5 transition-all ease-in-out delay-50 relative w-[100%] max-w-[550px] cursor-pointer z-10`}>
      <div className={`${type === "Visa" && "visa"} ${type === "Mastercard" && "mastercard"} ${type !== "Mastercard" && type !== "Visa" && "bg-white"} p-[30px] shadow-containersShadow flex justify-between`}>
        ...
        <div className={`flex flex-col justify-between items-end ${type === "Mastercard" ? "pt-0 pb-2" : "py-2"}`}>
          <img className={`m-0 ${type === "Visa" ? "bg-white h-[40px]" : "h-[90px]"} rounded`} src={type !== "" ? type : ""} alt="" />
          <p className={`${type === "Visa" ? "text-white" : "text-boldText"} text-bold w-[68.5px] font-semibold`}>
            {month === undefined || month.length === 0 ? "00" : month}
            {" "}
            /
            {" "}
            {year === undefined || year.length === 0 ? "00" : year}
          </p>
        </div>
      </div>

      <div className={`${type === "Visa" && "visa"} ${type === "Mastercard" && "mastercard"} ${type !== "Mastercard" && type !== "Visa" && "bg-white"} w-[100%] p-0 back absolute top-0 shadow-containersShadow`}>
        <div className="h-[55px] w-full bg-black mt-[30px]" />
