...
export default function CardForm() {
  ...
  const {
    name, number, month, year, type, ccv,
  } = cardInfo;
  ...
  const inactiveButtonCheck = () => {
    if (validNumber.valid
      ...
      && validMonth.valid
      && validYear.valid
      && validCcv.valid) setInactiveButton(false);
    else setInactiveButton(true);
  };

  //* Name
  const nameRegex = (name) => name.toString()
    .replace(/[^a-zA-Z ]/g, "");

  const changeName = (name) => {
    setRotate(false);
    name = nameRegex(name);
    if (name.length === 0) setValidName({ ...validName, valid: false });
    else setValidName({ ...validName, valid: true });
    setCardInfo({
      ...cardInfo,
      name,
    });
  ...
  //* Number
  const numberRegex = (number) => number.toString()
    .replace(/\s/g, "")
    .replace(/\D/g, "")
    .replace(/([0-9]{4})/g, "$1 ")
    .trim();
  ...
  const changeMonth = (month) => {
    setRotate(false);
    let newMonth;
    if (month < 10) newMonth = month.toString().slice(1, 2);
    else newMonth = month;
