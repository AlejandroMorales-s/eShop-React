  ...
  const handleSelectChange = (event) => {
    ...
    case 2:
      ...
      setProductsFiltered(products.filter((product) => {
        if (product.data.price > 1000 && product.data.price < 1500) return product;
        return null;
      }));
      break;
    case 3:
      setProductsFiltered(products);
      setProductsFiltered(products.filter((product) => {
        if (product.data.price > 1500 && product.data.price < 2000) return product;
        return null;
      }));
      break;
    case 4:
      setProductsFiltered(products);
      setProductsFiltered(products.filter((product) => {
        if (product.data.price > 2000 && product.data.price < 3000) return product;
        return null;
      }));
      break;
    case 5:
