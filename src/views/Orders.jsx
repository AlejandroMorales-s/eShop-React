...
export default function Orders() {
  ...
  return (
    <>
      ...
      {orders.map((order) => (
        <div key={order.id} className="border-2 border-gray bg-white dark:bg-darkBg flex flex-col gap-2 w-[95%] max-w-[1000px] m-auto my-5 dark:border-gray-grayDark p-2 rounded shadow-shadow">
          {order.products.map((product) => {
            const { name, price, images } = product.data;
            return (
              <div key={product.name} className="border-2 border-gray dark:border-gray-grayDark p-1.5 flex gap-2">
          ...
          <div className="flex justify-between">
            <p className="text-bold font-medium dark:text-gray">
              Arrives:
              {order.shipping.tomorrow ? "Tomorrow" : order.shipping.date}
            </p>
            <p className="text-bold font-medium dark:text-gray">
              Total: $
              {order.total}
